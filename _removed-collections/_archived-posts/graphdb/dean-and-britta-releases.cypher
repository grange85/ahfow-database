...
MERGE (dab)-[:RELEASED]->(r{{release.albumname | slugify | replace: "-",""}})
...
{% assign i = 1 %}{% for group in version.groups %}{% for track in group.tracks %}
...
SET r{{track.title | slugify | replace: "-", ""}}{{track.notes | slugify | replace: "-", ""}}{{version.version | slugify | replace: "-", ""}}.position = {{i}}{% assign i = i | plus: 1%}

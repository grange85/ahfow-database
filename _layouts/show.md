...
{%- assign bookmarks = bookmarks | where: 'status', 'OK' -%}
...
<div>
					...
					{% capture nowdate %}{{'now' | date: '%s'}}{% endcapture %}
                        ...
                        {% assign supports = page.ahfow.support | split: "|" %}
						<div>
							<div>
								<div>
									<div id="support">
							...
							<div>
								<div>
									...
									<div>
										...
										<li><a href="{{bookmark.url}}">{{- bookmark.title -}}</a><br>
											{%- if bookmark.notes -%}<em>{{- bookmark.notes -}}</em><br>{%- endif -%}
											<small>(added: {{- bookmark.date_added -}})</small>
										</li>
										{%- endfor -%}
										</ul>
									</div>
